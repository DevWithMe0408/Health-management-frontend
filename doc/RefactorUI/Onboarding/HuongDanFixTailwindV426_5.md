...
- [ ] `/admin/meal-config` — table border, header text màu đúng
- [ ] `/admin/system-config` — input focus border xanh
...
## 3. DELETE FILE CONFIG CŨ (OPTIONAL NHƯNG KHUYẾN NGHỊ)

### 3.1. `tailwind.config.ts` ở root project

File này hiện đang **chết** — không có effect gì. Để tránh tương lai dev khác hiểu nhầm là source of truth, có 2 lựa chọn:

**Option A — Xóa file (khuyến nghị):**
```bash
rm tailwind.config.ts
rm -f postcss.config.* 2>/dev/null   # nếu có postcss cũ
```

Tailwind v4 không cần config file riêng.

**Option B — Giữ lại với comment cảnh báo:**

Nếu muốn giữ làm reference cho future migration (vd thêm plugin), thay nội dung file thành:

```typescript
/**
 * DEPRECATED — Tailwind v4 không đọc file này nữa.
 *
 * Brand tokens đã được migrate sang `src/index.css` qua @theme directive.
 * File này giữ lại chỉ để reference khi cần thêm plugin advanced.
 *
 * Nếu cần thêm color/font mới, edit `src/index.css` @theme block.
 */
...
export default {
  ...
  plugins: [forms],  // chỉ giữ plugin form, các phần khác đã chuyển sang CSS
} satisfies Config;
```

→ **Mình khuyến nghị Option A** — xóa hẳn, đỡ confusion về sau.

### 3.2. `@tailwindcss/forms` plugin

Plugin này hiện được import trong `tailwind.config.ts`. Vì v4 không đọc config đó nữa, plugin sẽ không work.

Nếu project có dùng form styling từ plugin này (vd `<input type="checkbox">` style đẹp), cần re-enable trong CSS:

```css
/* Trong index.css, sau @theme block */
@plugin "@tailwindcss/forms";
```

**Verify cần thiết:** Check xem có ô input/select nào hiện đang dùng style của plugin form không.

```bash
# Trong project, tìm các form element raw không có Tailwind utility classes:
grep -rn '<input\|<select\|<textarea\|<checkbox' src/ --include="*.tsx" | head -10
```

Nếu kết quả là input có nhiều Tailwind class explicit (`className="rounded-md border ..."`) → không cần plugin, có thể skip.

Nếu thấy input/select chỉ có `className="..."` ngắn → có thể đang phụ thuộc plugin, cần thêm `@plugin` dòng trên.
...
Đề xuất 3 phase, làm tuần tự, KHÔNG nhảy:

### Phase A — Fix Tailwind v4 (BLOCKER, ~15 phút)
1. Edit `src/index.css` thêm `@theme` block (§1.2)
2. Restart vite (`Ctrl+C` → `npm run dev`)
3. Verify visual đã đúng:
   ...
   - [ ] Step 3: step indicator hiển thị node 1 đã done (xanh có check trắng)
   - [ ] Step 4: activity cards có checkmark khi chọn
4. Verify admin pages không bị break (§2.2)
5. **STOP — báo cáo cho user xem screenshot Phase A xong**, đợi confirm trước khi sang Phase B

### Phase B — Polish bonus 3 detail (~30 phút)
6. Update `WizardProgress.tsx` (§4.1) — node done shadow + size lớn hơn
7. Update `Step2Goal.tsx` (§4.2) — checkmark pop-in animation
8. Update `Step4Activity.tsx` (§4.3) — thay radio dot bằng check icon
9. Update `formStyles.ts` (§4.4) — input transition mượt hơn
10. Thêm keyframe `wizardPop` vào `index.css` (§4.2)
11. Verify screenshot lại từng step

### Phase C — Page transition (OPTIONAL, ~15 phút)
12. Update `OnboardingWizard.tsx` thêm framer-motion (§5)
13. Test chuyển bước 1→2→3→4→5: phải có slide effect mượt
14. Test "Quay lại": slide ngược (phải→trái)

### Phase D — Cleanup (OPTIONAL, ~5 phút)
15. Xóa `tailwind.config.ts` (§3.1 Option A) — file đã không còn effect
16. Nếu thấy form input trông xấu sau khi xóa plugin form (vd checkbox), thêm `@plugin "@tailwindcss/forms";` vào `index.css` (§3.2)
...
# Phase D
git commit -m "chore: remove deprecated tailwind.config.ts

Tailwind v4 không dùng file này nữa. Config đã migrate sang
src/index.css qua @theme directive."
```
...
- Fix: Thêm `@plugin "@tailwindcss/forms";` vào CSS (§3.2)
...
### 10.2. KHÔNG được đụng

- KHÔNG đổi component logic
- KHÔNG đổi prop interface của các Wizard components
- KHÔNG xóa file `tailwind.config.ts` ở Phase A — phải verify Phase A xong + admin pages OK rồi mới xóa ở Phase D
